...
#define LATCH_ENABLE_PIN GPIO_Pin_14
...
volatile int32_t 	pololuRightPWM = 0;
...
volatile float 		whitePercent;
volatile float 		blackPercent;
...
void calibrate(uint32_t val){
	...
	//uint32_t nrOfSamples = 8;
	//uint16_t tempADC = 0;

 // Calibrate minimum (WHITE).
	if(val == 0){
...
void setLED( ){
	...
	for(uint16_t i=0; i<8; i++){
		if(ADC_avgBuff[i]>3000){
			LED_data[1] += (1<<i);
		}
	}
	for(uint16_t i=0; i<4; i++){
		if(ADC_avgBuff[i+8]>3000){
			LED_data[0] += (1<<i);
		}
	}
	//if white
...
void regulator( ){
 ...
 // Decide upon former.
	if(isFindingLine){
		...
		stopTimer++;	
		if(*virtualSensorValue < 0.0){ drive_turn_right(150); }
		else{ drive_turn_left(150); }
	}
	else{
		pid( );
	}
...
/*void USART6_IRQHandler( ){
	...
	if (USART_GetITStatus(USART6, USART_IT_RXNE) != RESET) {
		...
		uint8_t income = USART_ReceiveData(USART6);
		GPIO_ToggleBits(GPIOD, GPIO_Pin_13);
		if(usartReceiving==true){
			if(uPnt==-1){
				nrOfBytes = income;
				uPnt = 0;	//GPIO_SetBits(GPIOD, GPIO_Pin_13);
				//if(income==0){//GPIO_SetBits(GPIOD, GPIO_Pin_14);}
			}
			else{
				usartIncoming[uPnt] = income;
				uPnt++;
				//GPIO_SetBits(GPIOD, GPIO_Pin_15);
				if(uPnt==(nrOfBytes)){

					uPnt = -1;
					usartReceiving = false;
					handleUsart = true;
				}
			}
		}
		else{
			if(income==usartStart){
				nrOfStart++;
				if(nrOfStart==3){
					nrOfStart=0;
					usartReceiving = true;//GPIO_SetBits(GPIOD, GPIO_Pin_12);					
				}
			}
		}				
	}
